...
* @Last Modified time: 2016-05-01 06:21:57
*/
'use strict'
...
module.exports = class { // 店铺优惠券 API
  ...
  * couponId  优惠券ID
  */
  delete (...couponIds) { // 删除优惠券
    const result = { success: [], fail: [] }
    for (let param of couponIds) {
      if (typeof param !== 'object') param = { couponId: param }
      try {
        this.vdian.api(param, 'delete.coupon')
        result.success.push(param.couponId)
      } catch (error) {
        result.fail.push({
  ...
  * couponId  优惠券ID
  */
  detail (param) { // 获取优惠券详情
    if (typeof param !== 'object') param = { couponId: param }
    return this.vdian.api(param, 'get.shop.coupon.detail')
  }
